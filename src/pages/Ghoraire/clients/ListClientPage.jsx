...
export default function ListClientPage() {
    ...
    const deleteItems = async (itemsIds) => {
        try {
            ...
            await fetchApi(`/api/client/${itemsIds}`, {
                method: "DELETE",
            });
            dispacth(
                setToastAction({
    ...
    const fetchSalle = useCallback(async () => {
        try {
              ...
              setLoading(true);
            let url = `${API_URL.LOCAL_URL}/api/client?`;
    ...
    return (
        <>
                    ...
                    <div className="rounded mt-3 pr-1 bg-white">
                        ...
                        >
                            <Column
                                ...
                                header='No'
                                sortable
                                body={(item) => (item ? item.customer_code : "-")}
                            />
                             <Column
                                field="customer_TIN"
                                header='NIF'
                                sortable
                                body={(item) => ( item?.customer_TIN ? item.customer_TIN : "-")}
                            />
                            ...
                            <Column
                                ...
                                header='Adresse'
                                sortable
                                body={(item) => (item ? item.customer_address : "-")}
                            />
            ...
            <AddClientPage setVisible={setVisible} visible={visible} fetchSalle={fetchSalle} />
            {/* ********modal pour editer un elemnt ******** */}
            <EditClientPage setElementEdit={setElementEdit} elementEdit={elementEdit} setVisibleModal={setVisibleModal} visibleModal={visibleModal} fetchSalle={fetchSalle} />
        </>
    );
}