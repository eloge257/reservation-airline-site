...
export default function ListCompaniesPage() {
    ...
    const deleteItems = async (itemsIds) => {
        try {
          ...
          const response =  await axios.delete(`${API_URL.LOCAL_URL}/api/company/${itemsIds}`);
    ...
    const fetchSalle = useCallback(async () => {
        try {
            setLoading(true);
            var url = `${API_URL.LOCAL_URL}/api/company?`;
    ...
    return (
        <>
                    ...
                    <div className="rounded mt-3 pr-1 bg-white">
                        ...
                        >
                            ...
                            <Column
                                ...
                                header='TIN'
                                sortable
                                body={(item) => (item ? item.tp_TIN : "-")}
                            />
                            <Column
                                field="Phone"
                                header='Phone'
                                sortable
                                body={(item) => (item ? item.tp_phone_number : "-")}
                            />
                            <Column
                                field="Adresse"
                                header='Adresse'
                                sortable
                                body={(item) => (item ? `${item.tp_address_province}/${item.tp_address_commune}/${item.tp_address_quartier}
                                    /${item.tp_address_avenue}/${item.tp_address_rue}` : "-")}
                            />
                            <Column
                                field="Secteur"
                                header='Secteur'
                                sortable
                                body={(item) => (item ? item.tp_activity_sector : "-")}
                            />
            ...
            {/* ******* modal pour ajouter les company  ******* */}
            <AddCompanyPage setVisible={setVisible} visible={visible} fetchSalle={fetchSalle} />
            <EditCompanyPage setElementEdit={setElementEdit} elementEdit={elementEdit} setVisibleModal={setVisibleModal} visibleModal={visibleModal} fetchSalle={fetchSalle} />
