...
import AddProduitPage from "./AddProduitPage";
import EditPoduitPage from "./EditPoduitPage";
...
export default function ListProduitPage() {
    ...
    const fetchSalle = useCallback(async () => {
        try {
            ...
            setLoading(true);
            let url = `${API_URL.LOCAL_URL}/api/produits?`;
    ...
    return (
        <>
                    ...
                    <div className="rounded mt-3 pr-1 bg-white">
                        ...
                        >
                             <Column
                                field="Fournisseur"
                                header='Fournisseur'
                                body={(item) => (item ? item.nomfournisseur : "-")}
                            />
                             <Column
                                field="Categorie"
                                header='Categorie'
                            ...
                            <Column
                                field="Nom"
                                header='Nom'
                                body={(item) => (item ? item.productName : "-")}
                            />
                            ...
                            <Column
                                field="UNITE"
                                header='unité'
                                body={(item) => (item ? item.nomunite : "-")}
                            />
                             <Column
                                field="cost"
                                header='Cost'
                                body={(item) => (item ? item.cost : "-")}
                            />
                            <Column
                                field="PT"
                                header='Prix total'
                                body={(item) => (item ? (item.price*item.quantity) : "")}
                            />
                            <Column
                                field=""
                                header="Action"
                                    ...
                                    return (
                                        <div className="d-flex">
                                            ...
                                            <Button
                                                ...
                                                severity="danger"
                                                className="ml-2"
                                                onClick={(e) => handleDeletePress(e, item.idproduct)}
                                            />
                                        </div>
                                    );
                                }}
            ...
            <AddProduitPage setVisible={setVisible} visible={visible} fetchSalle={fetchSalle} />
            {/* ********modal pour editer un elemnt ******** */}
            <EditPoduitPage setElementEdit={setElementEdit} elementEdit={elementEdit} setVisibleModal={setVisibleModal} visibleModal={visibleModal} fetchSalle={fetchSalle} />
        </>
    );
}